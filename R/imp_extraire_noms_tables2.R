...
#'
#' \dontrun{
#' imp_extraire_noms_tables (lignes_dump = mes_lignes_du_dump)
#' }
#'
#'
imp_extraire_noms_tables2 <- function(lignes_dump) {
  ...
  noms_tables <- lignes_dump[which(stringi::stri_detect_fixed(lignes_dump, "CREATE TABLE aspe."))] %>%
    stringr::str_replace_all(pattern = "CREATE TABLE aspe.", replacement = "") %>%
